...
if __name__ == "__main__":
    ...
    eval_gen = SimpleDataGenerator(data_reader, params.batch_size, lidar_files[:24], label_files[:24], calibration_files[:24])
    ...
    for i in range(loop_range):
        set_boxes.append(generate_bboxes_from_pred(occupancy[i], position[i], size[i], angle[i], heading[i],
                                                   classification[i], params.anchor_dims, occ_threshold=0.1))
    ...
    # NMS
    nms_boxes = rotational_nms(set_boxes, confidences, occ_threshold=0.1, nms_iou_thr=0.01)
    ...
    for seq_boxes, gt_label, gt0 in zip(nms_boxes, gt_gen, gt_gen0):
        ...
        print("---------- ---------------------- ")
        f=open('/home/ecurl/PP3/PointPillars/results/labels/'+scan_id,"w")
